...
export default function UnitDetails() {
  ...
  const handleApply = async (e) => {
    ...
    if (!moveInDate) {
      setError("Please select your target move-in date.");
      return;
    }
    if (!moveOutDate) {
      setError("Please select your target move-out date.");
      return;
    }
  ...
  if (loading)
    return <div className="p-5 text-center mt-5">Loading unit details...</div>;
  if (error && !unit)
    return (
      <div
        className="p-5 text-center mt-5 alert alert-danger mx-auto"
    ...
    >
      ...
      <header className="d-flex justify-content-between align-items-center px-4 py-3 bg-white border-bottom w-100">
        ...
        >
          ...
          <Link
            to="/contact"
            className="text-decoration-none text-muted d-none d-sm-block"
          >
            Contact Us
          </Link>
          <Link
          ...
          <div className="col-lg-5">
            ...
            >
              <div className="d-flex justify-content-between align-items-start mb-3">
                ...
                <span
                  className={`badge rounded-pill px-3 py-2 ${unit.status === "available" ? "bg-success" : "bg-danger"}`}
                >
              ...
              <p className="text-muted mb-4">
                {unit.description ||
                  "A beautiful and spacious unit perfect for comfortable living in Quezon City."}
              </p>

              <div className="row g-3 mb-5 text-muted">
                <div className="col-6 col-md-4 d-flex align-items-center gap-2">
                  <FiMaximize size={20} className="text-primary" />{" "}
                  <strong>{unit.sqm || "0"} sqm</strong>
                </div>
                <div className="col-6 col-md-4 d-flex align-items-center gap-2">
                  <FiHome size={20} className="text-primary" />{" "}
                  <strong>{unit.type || "Studio"}</strong>
                </div>
                <div className="col-6 col-md-4 d-flex align-items-center gap-2">
                  <FiMapPin size={20} className="text-primary" />{" "}
                  <strong>Floor {unit.floor || "1"}</strong>
                </div>
              </div>
