...
export default function TenantDashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        try {
          const tenantRes = await axios.get('http://localhost:8000/api/dashboard/tenant', config);
        ...
        try {
          const appRes = await axios.get('http://localhost:8000/api/applications/my/applications', config);
          // Depende sa backend format niyo, kunin ang array ng applications
          const apps = appRes.data.applications || appRes.data.data || appRes.data || [];
  ...
  if (loading) return <div className="p-5 text-center">Loading your account info...</div>;
    ...
    <div className="p-4 bg-light min-vh-100">
      <div className="mb-4">
        ...
        <p className="text-muted">
          {hasActiveLease ? 'Here is the summary of your rental account.' : 'Here is the status of your account.'}
        </p>
      </div>
      ...
      {hasActiveLease ? (
        ...
        <div className="row mt-4">
          <div className="col-md-8 mx-auto">
            <div className={`card border-0 shadow p-5 text-center border-top border-4 ${
              latestApp.status === 'approved' ? 'border-success' : 
              latestApp.status === 'denied' || latestApp.status === 'rejected' ? 'border-danger' : 'border-warning'
            }`}>
              <h5 className="text-muted mb-3">
                Application for Unit: <span className="fw-bold text-dark">{latestApp.unitId?.unitNumber || latestApp.unitId?.name || 'Selected Unit'}</span>
              </h5>
              
              {latestApp.status === 'approved' && (
                <>
                  <h2 className="fw-bold text-success mb-3">Application Approved! 🎉</h2>
                  <p className="text-muted fs-5 mb-0">
                    Congratulations! Your application has been approved by the landlord. Please wait while we process and activate your official lease contract. You will be able to view your payments and unit details here soon.
                  </p>
                </>
              )}

              {(latestApp.status === 'denied' || latestApp.status === 'rejected') && (
                <>
                  <h2 className="fw-bold text-danger mb-3">Application Denied</h2>
                  <p className="text-muted fs-5 mb-0">
                    We're sorry, but your application for this unit was not approved. You may browse other available units and try submitting a new application.
                  </p>
                </>
              )}
              ...
              {(!latestApp.status || latestApp.status === 'pending') && (
                <>
                  <h2 className="fw-bold text-warning mb-3">Pending Review ⏳</h2>
                  <p className="text-muted fs-5 mb-0">
                    Your application is currently being reviewed by the landlord. Please check back later for updates.
                  </p>
                </>
              )}
            </div>
        ...
        <div className="row mt-4">
          <div className="col-md-8 mx-auto">
            <div className="card border-0 shadow-sm p-5 text-center">
              <h3 className="fw-bold text-secondary mb-3">No Active Applications</h3>
              <p className="text-muted fs-5 mb-0">
                You don't have any active leases or pending applications right now. Browse our available units to start your journey!
              </p>
            </div>
          </div>
        </div>
