...
export default function Maintenance() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      try {
        const leaseRes = await axios.get(`${API_URL}/api/leases/my/lease`, {
        ...
        if (leaseRes.data.lease && leaseRes.data.lease.unitId) {
          const fetchedUnitId =
            leaseRes.data.lease.unitId._id || leaseRes.data.lease.unitId;
          setUnitId(fetchedUnitId);
        }
      } catch (leaseErr) {
        console.error(
          "No active lease found or error fetching lease.",
          leaseErr,
        );
      }

      // B. Kunin ang Maintenance Requests
      const reqRes = await axios.get(`${API_URL}/api/maintenance/my/requests`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return (
          <span className="badge bg-warning-subtle text-warning rounded-pill px-3 py-1">
            OPEN
          </span>
        );
    }
  ...
  const getPriorityBadge = (priority) => {
    switch (priority) {
      case "high":
        return <span className="text-danger fw-bold small">High</span>;
      case "medium":
        return <span className="text-warning fw-bold small">Medium</span>;
      default:
        return <span className="text-success fw-bold small">Low</span>;
    }
  };
      ...
      >
        <div className="mb-4">
          ...
          <p className="text-muted small">
            Report issues in your unit and track repair progress.
          </p>
        </div>
          ...
          <div className="col-md-8 mx-auto">
            <div className="card border-0 shadow-sm p-5 text-center">
              ...
              <p className="text-muted fs-5 mb-0">
                Maintenance requests are only available for official tenants
                with an active lease. Please wait for your application to be
                approved.
              </p>
            </div>
          </div>
          ...
          <div className="card border-0 shadow-sm">
            <div className="card-body p-4">
              <h5 className="fw-bold mb-3 d-flex align-items-center gap-2">
                <FiTool className="text-primary" /> New Request
              </h5>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  >
                    ...
                    <option value="medium">
                      Medium (Annoyance but livable)
                    </option>
                    <option value="high">
                      High (Emergency / Safety Hazard)
                    </option>
                  </select>
                </div>
                ...
                <button
                  type="submit"
                  className={`btn btn-primary w-100 fw-bold py-2 ${!unitId ? "disabled bg-secondary" : ""}`}
                  style={{
                    backgroundColor: unitId ? "#0ea5e9" : "",
                    border: "none",
                  }}
                  disabled={submitLoading || !unitId}
                >
