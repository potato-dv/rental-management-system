        ...
        <div className="graph-card card shadow-sm">
          ...
          <div className="bar-chart">
            {Array.isArray(revenueData) && revenueData.map((item) => (
              <div className="bar-item" key={item.month}><span>{item.amount}</span><div className="bar-bg"><div className="bar-fill" style={{ height: item.height }}></div></div><p>{item.month}</p></div>
            ))}
          </div>
        </div>
        <div className="graph-card card shadow-sm">
