...
export const LandingPage = () => {
  ...
  const [showDeleteMaintenanceModal, setShowDeleteMaintenanceModal] =
    useState(false);
  ...
  useEffect(() => {
    const fetchAllDashboardData = async () => {
      try {
        ...
        const occupiedCount = fetchedUnits.filter(
          (u) => u.status?.toLowerCase() === "occupied",
        ).length;
        const activeTenantsCount = fetchedTenants.length;
        const pendingMaintenanceCount = fetchedMaintenance.filter((m) => {
        ...
        const monthlyRev = fetchedPayments.reduce((acc, curr) => {
          if (
            curr.status?.toLowerCase() === "paid" ||
            curr.status?.toLowerCase() === "verified"
          ) {
            const dateVal = curr.date || curr.createdAt || new Date();
            const month = new Date(dateVal).toLocaleString("default", {
              month: "short",
            });
        ...
        setRevenueData(
          chartData.length > 0
            ? chartData
            : [{ month: "No Data", amount: "₱0", height: "10%" }],
        );
      } catch (error) {
  ...
  const handleOpenEditUnit = (unit) => {
    ...
    setUnitFormData({
      ...
      images:
        unit.images && unit.images.length > 0 ? unit.images.join(", ") : "",
      floor: unit.floor || "",
  ...
  const handleUnitSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        floor: unitFormData.floor,
        description: unitFormData.description,
        images: unitFormData.images
          ? unitFormData.images
              .split(",")
              .map((url) => url.trim())
              .filter((url) => url !== "")
      ...
      if (editingUnitId) {
        ...
        setUnitsData((prev) =>
          prev.map((u) =>
            (u._id || u.id) === editingUnitId ? res.data.data || res.data : u,
          ),
        );
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setUnitsData((prev) =>
        prev.filter((u) => (u._id || u.id) !== unitToDelete),
      );
      setStats((prev) => ({ ...prev, unitsTotal: prev.unitsTotal - 1 }));
  ...
  const confirmDeleteTenant = async () => {
    ...
    try {
      ...
      setTenantsData((prev) =>
        prev.filter((t) => (t._id || t.id) !== tenantToDelete),
      );
  ...
  const handleOpenMaintenance = (item) => {
    ...
    setMaintenanceStatus(item.status || "open");
  ...
  const handleDeleteMaintenanceClick = (id) => {
  ...
  const confirmDeleteMaintenance = async () => {
    ...
    try {
      ...
      setMaintenanceData((prev) =>
        prev.filter((m) => (m._id || m.id) !== maintenanceToDelete),
      );
  ...
  const confirmDeleteReport = async () => {
    ...
    try {
      ...
      setReportsData((prev) =>
        prev.filter((r) => (r._id || r.id) !== reportToDelete),
      );
  ...
  const handleApproveApplication = async (appId) => {
    try {
      ...
      setApplicationsData((prev) =>
        prev.map((app) =>
          (app._id || app.id) === appId ? { ...app, status: "approved" } : app,
        ),
      );
  ...
  const handleDenyApplication = async (appId) => {
    try {
      ...
      setApplicationsData((prev) =>
        prev.map((app) =>
          (app._id || app.id) === appId ? { ...app, status: "denied" } : app,
        ),
      );
      showNotification("Application denied and removed from list.", "success");
    } catch (error) {
      showNotification(
        error.response?.data?.message || "Failed to deny application.",
        "error",
      );
    }
  };
  const handleVerifyPayment = async (paymentId) => {
    try {
      ...
      setPaymentsData((prev) =>
        prev.map((p) =>
          (p._id || p.id) === paymentId ? { ...p, status: "verified" } : p,
        ),
      );
  ...
  const handleApplicantChange = (e) => {
    ...
    const selectedApp = applicationsData.find(
      (app) => (app._id || app.id) === appId,
    );
    if (selectedApp) {
      const tObj =
        selectedApp.tenantId || selectedApp.tenant || selectedApp.user;
      const uObj = selectedApp.unitId || selectedApp.unit;
      const tId =
        typeof tObj === "object" && tObj !== null ? tObj._id || tObj.id : tObj;
      const uId =
        typeof uObj === "object" && uObj !== null ? uObj._id || uObj.id : uObj;
      const monthlyRent = uObj?.rent || uObj?.price || 0;
      const uName = uObj?.unitNumber || uObj?.name || "Assigned Unit";
      let formattedDate = "";
      if (selectedApp.moveInDate) {
        try {
          formattedDate = new Date(selectedApp.moveInDate)
            .toISOString()
            .split("T")[0];
        } catch (err) {}
      }
      let formattedEndDate = "";
      if (selectedApp.moveOutDate) {
        try {
          formattedEndDate = new Date(selectedApp.moveOutDate)
            .toISOString()
            .split("T")[0];
        } catch (err) {}
      }
  ...
  const handleLeaseSubmit = async (e) => {
    ...
    try {
      const payload = {
        tenantId: leaseFormData.tenantId,
        unitId: leaseFormData.unitId,
        monthlyRent: Number(leaseFormData.rent),
      ...
      setLeasesData((prev) => [...prev, newLease]);
  ...
  const getMinLeaseEndDate = (startDate) => {
    if (!startDate) return "";
    ...
    <div className="admin-page d-flex min-vh-100">
      <aside className="sidebar d-flex flex-column justify-content-between text-white">
        ...
        <div className="sidebar-bottom">
          ...
          <button
            className="menu-button btn logout-menu"
            onClick={() => {
      ...
      <main className="main-content flex-grow-1 position-relative">
        ...
        {page === "Units" && (
          <UnitsTab
            ...
            handleOpenAddUnit={handleOpenAddUnit}
            handleOpenEditUnit={handleOpenEditUnit}
        ...
        {page === "Tenants" && (
          <TenantsTab
            ...
            leasesData={leasesData}
            handleDeleteTenantClick={handleDeleteTenantClick}
          />
        )}
        {page === "Applications" && (
          <ApplicationsTab
        ...
        {page === "Maintenance" && (
          <MaintenanceTab
            ...
            handleDeleteMaintenanceClick={handleDeleteMaintenanceClick}
          />
        )}
        ...
        {page === "Leasing" && (
          <LeasingTab
            leasesData={leasesData}
            setLeaseFormData={setLeaseFormData}
            setShowLeaseModal={setShowLeaseModal}
          />
        )}
        ...
        {page === "Help/Supports" && (
          <ReportsTab
            ...
            handleDeleteReportClick={handleDeleteReportClick}
          />
        )}
              ...
              <form onSubmit={handleLeaseSubmit}>
                <div className="mb-3">
                  ...
                  <select
                    className="form-select"
                    required
                    value={leaseFormData.applicationId}
                ...
                <div className="mb-3">
                  ...
                  <input
                    ...
                    disabled
                    value={
                      leaseFormData.applicationId
                        ? `${leaseFormData.unitDisplay} - ₱${leaseFormData.rent.toLocaleString()}`
                ...
                <div className="row mb-4">
                  <div className="col-6">
                    <label className="fw-bold small mb-1">Start Date</label>
                    <input
                      type="date"
                      className="form-control"
                      required
                      ...
                      onChange={(e) =>
                        setLeaseFormData({
                          ...
                          endDate:
                            ...
                            getMinLeaseEndDate(e.target.value) &&
                            new Date(leaseFormData.endDate) <
                              new Date(getMinLeaseEndDate(e.target.value))
                              ? ""
                              : leaseFormData.endDate,
                        })
                      }
                  ...
                  <div className="col-6">
                    <label className="fw-bold small mb-1">End Date</label>
                    <input
                      type="date"
                      className="form-control"
                      required
                      ...
                      min={getMinLeaseEndDate(leaseFormData.startDate)}
                    />
                ...
                <div className="d-flex justify-content-end gap-2 border-top pt-3">
                  ...
                  <button
                    ...
                    disabled={!leaseFormData.applicationId}
                  >
                    Create Lease
                  </button>
                </div>
        ...
        {showDeleteTenantModal && (
          ...
          >
            ...
            >
              ...
              <h4 className="text-danger fw-bold mb-3">Remove Tenant</h4>
              <p className="text-muted">
                Are you sure you want to remove this tenant? This will also
                delete their active lease and pending payments.
              </p>
              <div className="d-flex justify-content-center gap-3 mt-4">
                <button
                  className="btn btn-secondary px-4"
        ...
        {showMaintenanceModal && (
          ...
          >
            ...
            >
              <h4 className="mb-1 fw-bold">Update Request</h4>
              <p className="text-muted small mb-4">
                Change the status of this maintenance request.
              </p>
              <form onSubmit={handleUpdateMaintenance}>
                <div className="mb-3">
                  <label className="fw-bold small mb-1">Issue / Title</label>
                  <input
                    type="text"
                    className="form-control bg-light"
                    disabled
        ...
        {showReportModal && (
          ...
          >
            ...
            >
              <h4 className="mb-1 fw-bold">Update Report</h4>
              <p className="text-muted small mb-4">
                Change the status of this tenant report.
              </p>
              <form onSubmit={handleUpdateReport}>
                <div className="mb-3">
                  <label className="fw-bold small mb-1">Title</label>
                  <input
                    type="text"
                    className="form-control bg-light"
                    disabled
        ...
        {notification.show && (
          ...
          >
            ...
            >
              <div
                className={`mb-3 ${notification.type === "success" ? "text-success" : "text-danger"}`}
              >
